...
//        $this->date    = time();
    ...
    function getImgFB($selectedImg) {
        ...
        $query = $this->db->query('SELECT DISTINCT med.file, dist.value
                                   FROM media med, media_descriptor mdesc, distance dist
                                   WHERE med.id_media = mdesc.id_media AND mdesc.id_descr IN (dist.id_descr1, dist.id_descr2)
    ...
    function setImg($fileName, $keywords) {
        ...
        $this->db->query('INSERT INTO media_descriptor (id_media) VALUES("' . $id_media . '");');

        $query_id_descr = $this->db->query('SELECT id_descr FROM media_descriptor WHERE id_media = "' . $id_media . '";');
        $rowDescr = $query_id_descr->row();
        $id_descr = $rowDescr->id_descr;
        $this->db->query('INSERT INTO descriptor (id_descr) VALUES("' . $id_descr . '");');
    ...
    function setImgDist($distanceTab, $nameTab) {
        ...
        foreach ($distanceTab as $fileName1 => $strDist) {
            ...
            $query_id_descr1 = $this->db->query('SELECT id_descr FROM media_descriptor WHERE id_media = ' . trim($id_media1) . ';');
            ...
            foreach ($distTab as $indexDistTab => $valueDist) {
                $fileName2 = $nameTab[$indexDistTab];
                //TO DO: SQL
                ...
                $query_id_descr2 = $this->db->query('SELECT id_descr FROM media_descriptor WHERE id_media = ' . trim($id_media2) . ';');
                $row_id_descr2 = $query_id_descr2->row();
                $id_descr2 = $row_id_descr2->id_descr;


                $this->db->query('INSERT INTO distance (id_descr1, id_descr2, value) VALUES(' . trim($id_descr1) . ', ' . trim($id_descr2) . ', ' . trim($valueDist) . ');');
            }
        }
    }
